...
if (flashtrigger)
{
    ...
    if (flashpercent == 0.1)
    {
        ...
        if (instance_exists(ENGINE))
        {
            ...
            if ((itemchoice + global.susiehp) > 0 && global.susiehp <= 0)
                greenhp.up = true;
            
            global.susiehp += itemchoice;
            global.susiehp = clamp(global.susiehp, -999, 190);
            greenhp.image_blend = c_lime;
            greenhp.value = itemchoice;
            
            if (global.susiehp >= global.susiemaxhp)
