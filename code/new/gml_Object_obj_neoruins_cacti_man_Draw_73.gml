...
if (talking)
{
    ...
    switch (dialogue)
    {
        case 1:
            texttalk = true;
            current_dialogue = format_text(translation_get_string("gml_Object_obj_neoruins_cacti_man_Draw_73_0"), 31);
            typing_speed = 1;
            break;
        case 2:
            texttalk = true;
            current_dialogue = format_text(translation_get_string("gml_Object_obj_neoruins_cacti_man_Draw_73_1"), 31);
            typing_speed = 1;
            break;
        default:
            current_dialogue = translation_get_string("gml_Object_obj_neoruins_cacti_man_Draw_73_2");
            break;
    }
    ...
    if (!text_complete && !pause_for_punctuation)
    {
        ...
        while (typing_timer >= 1)
        {
            ...
            if (letter_index < string_length(current_dialogue))
            {
                ...
                var following_char = ((letter_index + 2) <= string_length(current_dialogue)) ? string_char_at(current_dialogue, letter_index + 2) : translation_get_string("gml_Object_obj_neoruins_cacti_man_Draw_73_3");
