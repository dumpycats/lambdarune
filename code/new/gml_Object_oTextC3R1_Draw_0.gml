...
if (global.dialogue == 13)
{
    text_x = global.charactertalking ? (camera_get_view_x(view_camera[1]) + 174) : (camera_get_view_x(view_camera[1]) + 58);
    
    if (global.dialoguebottom)
        text_y = camera_get_view_y(view_camera[1]) + 338;
    else if (global.dialoguetop)
        text_y = camera_get_view_y(view_camera[1]) + 30;
}
