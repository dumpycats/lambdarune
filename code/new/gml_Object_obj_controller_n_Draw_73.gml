...
if (!gamepad_is_connected(0) && false)
{
    ...
    if (t_touch_id != -1)
    {
        if (device_mouse_check_button(t_touch_id, mb_left))
        {
            _touch_x = device_mouse_x(t_touch_id);
            _touch_y = device_mouse_y(t_touch_id);
            _touch_active = true;
        }
        else
        {
    ...
    if (touchtimer == 0)
    {
        keyboard_key_press(ord("N"));
        image_alpha = 1.5;
    }
    else if (releasetimer == 0)
    {
        keyboard_key_release(ord("N"));
    }
}
